...
| [`97 - scan main`](sonar-main-scan.yml) | Scans `main` on every push (and on manual dispatch), so `main` gets a computed Sonar quality gate instead of `NONE`: every other scan is a PR analysis, and the gate's new-code conditions need a second `main` analysis as their baseline. Not part of either path, and its job is deliberately not named `gate`, so it can never block or satisfy a pull request. |
